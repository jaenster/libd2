...
pub const Font = struct {
    ...
    /// line would use.
    ///
    /// Breaking is greedy and only at spaces, which is what the game's own word wrap does — a word
    /// longer than the box overhangs rather than being cut.
    pub fn drawWrappedCentered(
        ...
        while (rest.len != 0) {
            var take = rest.len;
            // Walk back to the last space that fits. If none fits, the first word goes out whole.
            while (take != 0 and self.measure(rest[0..take]) > width) {
                const space = std.mem.lastIndexOfScalar(u8, rest[0..take], ' ') orelse break;
                take = space;
            }
            if (take == 0) take = rest.len;

            self.drawCentered(canvas, pal, cx, baseline, rest[0..take], shift);
