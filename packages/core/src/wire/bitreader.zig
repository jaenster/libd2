//! LSB-first bit reader — a faithful stand-in for D2 1.14d Fog::BitBuffer (isSave=0 read side).
//!
//! Bytes are consumed in order; within each byte the least-significant bit is read first. This
//! is the same bit packing D2 uses for the item stream (0x9C/0x9D payloads), the life packets
...
pub const BitReader = struct {
    ...
    /// Read a fixed-length run of `count` chars, each `char_bits` wide (D2 packs names/codes as
    /// 7- or 8-bit chars). Writes into `out` (up to out.len), returns the count actually read.
    pub fn readChars(self: *BitReader, out: []u8, count: usize, char_bits: u6) usize {
        var i: usize = 0;
        while (i < count and i < out.len) : (i += 1) out[i] = @truncate(self.read(char_bits));
        return i;
    }
};
...
test "LSB-first ordering within a byte" {
    var r = BitReader.init(&[_]u8{0xB2}); // bits LSB..MSB: 0 1 0 0 1 1 0 1
    try std.testing.expectEqual(@as(u32, 0), r.read(1));
    try std.testing.expectEqual(@as(u32, 1), r.read(1));
    try std.testing.expectEqual(@as(u32, 0), r.read(2));
    try std.testing.expectEqual(@as(u32, 0xB), r.read(4));
}
...
test "cross-byte unaligned read" {
    // two bytes: read 4, then 8 spanning the byte boundary.
    var r = BitReader.init(&[_]u8{ 0xF0, 0x0F });
    try std.testing.expectEqual(@as(u32, 0x0), r.read(4)); // low nibble of 0xF0
    try std.testing.expectEqual(@as(u32, 0xFF), r.read(8)); // high nibble of byte0 + low nibble of byte1
}

test "signed sign-extension" {
    var r = BitReader.init(&[_]u8{0b0000_0111}); // 3-bit value 0b111 = -1
    try std.testing.expectEqual(@as(i32, -1), r.readSigned(3));
    var r2 = BitReader.init(&[_]u8{0b0000_0011}); // 3-bit value 0b011 = 3
    try std.testing.expectEqual(@as(i32, 3), r2.readSigned(3));
}

test "reading past end yields zeros, advances position" {
    var r = BitReader.init(&[_]u8{0xFF});
    try std.testing.expectEqual(@as(u32, 0xFF), r.read(8));
    try std.testing.expectEqual(@as(usize, 0), r.bitsLeft());
    try std.testing.expectEqual(@as(u32, 0), r.read(4)); // past end
    try std.testing.expectEqual(@as(usize, 12), r.bit_pos);
}
