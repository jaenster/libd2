...
pub const Stat = enum(u16) {
    ...
    poisonresist = 45,
    maxpoisonresist = 46,
    // Skill-boost stats (ItemStatCost 1.14d). addclassskills(83) bumps one class' whole tree;
    // allskills(127) bumps every skill. The host reads these summed totals to raise a build's
    // effective skill levels. addclassskills carries a class id in its stat param (unused here).
    addclassskills = 83,
    allskills = 127,
    // Flat +elemental-damage stats (ItemStatCost 1.14d). Skills add these to their base
    // element damage; combat reads them when resolving an elemental hit. min/max pairs.
    firemindam = 48,
    firemaxdam = 49,
    lightmindam = 50,
    lightmaxdam = 51,
    ...
    item_attackertakesdamage = 78, // flat physical returned to a melee attacker (Iron Maiden / gear)
    thorns_percent = 131, // % of the attacker's physical damage returned (Thorns aura)
