...
//! Header row maps column name -> index; data rows are slices into the source.
//! Empty fields read as 0 for integer accessors (D2's convention).
...
pub const Table = struct {
    ...
    pub fn parse(gpa: std.mem.Allocator, src: []const u8) !Table {
        ...
        while (lines.next()) |raw| {
            ...
            // D2 marks end-of-table with an "Expansion" sentinel row; keep all
            // real rows. A lone trailing newline is already skipped above.
        ...
        // re-point arena into the struct copy
        t.arena = arena;
        return t;
    }
...
test "parse header and integer access" {
    ...
    try testing.expectEqual(@as(?usize, null), t.findByInt("Id", 99));
}
