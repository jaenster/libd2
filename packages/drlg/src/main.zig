...
pub fn main(init: std.process.Init.Minimal) !void {
    ...
    if (std.mem.eql(u8, cmd, "ds1")) {
        const path = args.next() orelse {
            std.debug.print("usage: d2-drlg ds1 <file.ds1>\n", .{});
            return;
        };
    ...
    if (std.mem.eql(u8, cmd, "amdiag")) {
        ...
        var w: i32 = 64; var h: i32 = 64;
        if (tlv.size_x > 0 and tlv.size_y > 0) { w = @intCast(tlv.size_x); h = @intCast(tlv.size_y); }
        const lvl = try drlglib.generate(&ctx, seed, @enumFromInt(level_id), .normal, .{ .x = 0, .y = 0, .w = w, .h = h });
        defer lvl.deinit();
        var fhit: usize = 0; var fmiss: usize = 0; var whit: usize = 0; var wmiss: usize = 0;
            ...
            for (d.wall_layers) |wl| {
                ...
                while (i < n) : (i += 1) {
                    ...
                    if (amt.lookup(lt, orient, mn, sub, 0) >= 0) whit += 1 else {
                        ...
                        const e = wmisses.getOrPutValue(gpa, k, 0) catch continue; e.value_ptr.* += 1;
                    }
                }
            }
        }
        ...
        var it = wmisses.iterator(); var shown: usize = 0;
        while (it.next()) |e| { if (shown >= 15) break; const k = e.key_ptr.*;
            std.debug.print("  o={d} m={d} s={d}  x{d}\n", .{ (k>>32)&0xff, (k>>16)&0xffff, k&0xffff, e.value_ptr.* }); shown += 1; }
        return;
    }
