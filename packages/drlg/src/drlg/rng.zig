...
//! high = bits 32..63).
//!
//! The LCG math itself lives once, in d2-core's `rng.Seed`. This file is the
//! recon-named façade the transformed closure calls: same entry points, same
//! D2SeedStrc signatures, with a two-word conversion at the boundary.
//!
...
//! now covering the D2SeedStrc <-> Seed conversion as well as the draw itself.

const structs = @import("structs.zig");
const D2SeedStrc = structs.D2SeedStrc;
const core = @import("../rng.zig");

/// D2SeedStrc -> the canonical seed: the same two 32-bit halves, signed in the
/// recon struct and unsigned in d2-core.
inline fn toSeed(seed: D2SeedStrc) core.Seed {
    return .{ .low = @bitCast(seed.nSeedLow), .high = @bitCast(seed.nSeedHigh) };
}

/// The canonical seed -> D2SeedStrc.
inline fn fromSeed(s: core.Seed) D2SeedStrc {
    return .{ .nSeedLow = @bitCast(s.low), .nSeedHigh = @bitCast(s.high) };
}
...
pub fn getModuloFromSeed(pSeed: *D2SeedStrc, nModulo: u32) u32 {
    return randomNumberSelector(pSeed, nModulo);
}
...
test "sEEDNEXT advances exactly like the canonical Seed.next" {
    const lows = [_]u32{ 0, 1, 7, 12345, 0xdeadbeef, 0xffffffff };
    const highs = [_]u32{ 0, 0x29a, 666, 0xffffffff };
    for (lows) |lo| {
        for (highs) |hi| {
            const a = sEEDNEXT(.{ .nSeedLow = @bitCast(lo), .nSeedHigh = @bitCast(hi) });
            var b = ref.Seed.init(lo, hi);
            _ = b.next();
            try std.testing.expectEqual(b.low, @as(u32, @bitCast(a.nSeedLow)));
            try std.testing.expectEqual(b.high, @as(u32, @bitCast(a.nSeedHigh)));
