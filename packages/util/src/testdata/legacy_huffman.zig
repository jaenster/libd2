//! Reference implementation used only to cross-check `huffman.zig`.
//!
//! This is the flattened decode table + decoder that has been decoding live 1.14d GS
//! traffic in the clientless bot (jaenster/D2PacketBased lineage). It stores the code
//! length alongside every symbol instead of looking it up, so its table layout is an
//! independent one — agreeing with it over the whole code space is real evidence that
//! the table `huffman.Table.build` derives from the engine's code lengths is right.
const std = @import("std");

const CharIndex = [256]u16{
...
const BitMasks = [16]u32{ 0x0000, 0x0001, 0x0003, 0x0007, 0x000F, 0x001F, 0x003F, 0x007F, 0x00FF, 0x01FF, 0x03FF, 0x07FF, 0x0FFF, 0x1FFF, 0x3FFF, 0x7FFF };

/// Decompress D2GS huffman `in` into `out`; returns the number of decompressed bytes,
/// or null on overflow. Mirrors DecompressDataContent (MSB-first 32-bit bit buffer).
pub fn decompress(in: []const u8, out: []u8) ?usize {
    var b: u32 = 0;
    var size: usize = in.len;
    var inptr: usize = 0;
    var maxcnt: usize = out.len;
    var outptr: usize = 0;
    var cnt: u32 = 0x20;
    while (true) {
        if (cnt >= 8) {
            while (size > 0 and cnt >= 8) {
                cnt -= 8;
                size -= 1;
                b |= @as(u32, in[inptr]) << @intCast(cnt);
                inptr += 1;
            }
        }
        const index: usize = CharIndex[b >> 0x18];
        const a: u32 = CharTable[index];
        const d: u32 = (b >> @intCast(0x18 - a)) & BitMasks[a];
        const c: u32 = CharTable[index + 2 * d + 2];
        cnt += c;
        if (cnt > 0x20) return out.len - maxcnt;
        if (maxcnt == 0) return null;
        maxcnt -= 1;
        out[outptr] = CharTable[index + 2 * d + 1];
        outptr += 1;
        b <<= @intCast(c & 0xFF);
    }
}
